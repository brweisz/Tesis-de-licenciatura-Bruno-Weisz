...
\begin{enumerate}
    ...
    \item Sean $a \in G, e \text{ elemento neutro de }G, d \in \mathbb{Z}^+$, si $d$ es el menor número tal que $a^d = e$ decimos que $d$ es el \textbf{orden} de $a$ en $(G, \cdot)$. Si no existe tal $d$ entonces decimos que $a$ es de orden \textit{infinito}.
...
\begin{itemize}
    ...
    \item El producto de $p(x)$ y $q(x)$ se obtiene a través de la multiplicación formal de sus símbolos, distribuyendo la suma. De esto se deduce que el grado resultante es igual a la suma de los grados de $p$ y $q$. Nuevamente notamos que la operación de producto $a_i \cdot b_j$ es sobre el cuerpo $\Fp$.
...
La intuición detrás de esto está dada por la propiedad de los polinomios de grado $n$ tienen como mucho $n-1$ puntos críticos, por lo tanto 2 polinomios con coeficientes en $\Fp$ se pueden cruzar a lo sumo en $n$ puntos. Podemos ver una intuición de esto en la figura \ref{fig:schwartz-zippel-intuicion}.
...
\label{fig:schwartz-zippel-intuicion}
\end{figure}
...
La dificultad ECDLP es similar a la del DLP para grupos genéricos. Sin embargo, hay algunas curvas elípticas conocidas para las cuales existen algoritmos más eficientes para resolver ECDLP.
...
\noindent donde $p$ es un número primo de 381 bits.

Esta curva posee la propiedad de que calcular \textbf{\textit{pairings}}\cite{galbraith2008pairings} es una operación poco costosa respecto a otras curvas. Ahora bien, ¿qué son los pairings? No voy a presentar una definición formal, más bien enunciar algunas de las propiedades que nos interesan en el contexto de los SNARKs.
