...
En esencia lo que queremos hacer es traducir un “dialecto'' a otro preservando la semántica de los programas. El código ACIR tiene 
\begin{itemize}
...
\end{itemize}
Lo primero que notamos es que dado un programa $P$ de ACIR, todos los witnesses de $P$ van a tener un target equivalente en un circuito de Plonky2 $C$ que tenga la misma semántica. Esto no va a ocurrir a la inversa, ya que un opcode de ACIR podría tener una implementación arbitrariamente compleja en Plonky2 que requiera el uso de muchos más targets. Por ende, si $W$ es el conjunto de witnesses de $P$ y $T$ el conjunto de targets en $C$, vamos a tener una función inyectiva 
$$witness\_target\_map: W \rightarrow T$$
que asigna a cada witness de $P$ un target de $C$. Concretamente, esta función va a ser implementada con un diccionario que tendremos que mantener durante toda la generación de C. ¿Por qué es necesario esto? Hay varios motivos para tener este mapeo:
\begin{enumerate}
    ...
    que asigna a los targets de $C$ un valor y también es representada como un diccionario. Pero ¿cómo sabemos cuál es el valor correspondiente a un target? Para resolver esto vamos a tener que usar la información de las funciones 
    $$witness\_target\_map: W \rightarrow T \text{  y  } val: W \rightarrow \Fp$$ 
    Con ambas, dado un witness $w \in P$ podemos saber cual es su target  correspondiente en $C$ y su valor en una ejecución concreta, por lo tanto también podemos construir la función $partial\_witness$ para que 
    $$partial\_witness(witness\_target\_map(w)) = val(w)$$ 
    Es importante notar que la función $partial\_witness$ solo involucra a los targets que tienen una correspondencia directa con un witness.
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{verbatim}
    private parameters indices : PRIV
    public parameters indices : PUB
...
o más simplemente como el sistema de ecuaciones
\begin{figure}[H]
	\centering
	\begin{minipage}[c]{0.6\textwidth}
		\centering
...
Estas ecuaciones sirven para restringir a la variable $equal$ a que sea un valor binario: $0$ si $x$ e $y$ son distintos y $1$ si son iguales. 
\begin{enumerate}
...
Internamente, el Generator recibe 1 target \texttt{full\_number} con su valor asociado y $N$ targets para los limbs ($l_0, \cdots, l_{N-1}$). Su trabajo es calcular el valor de los $N$ limbs. El algoritmo puede verse en el pseudocódigo de \ref{alg:descomposicion_limbs}.
