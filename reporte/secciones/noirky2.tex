...
\chapter{Noirky2}
Haber recorrido Noir y Plonky2 nos permite re-definir el problema que estamos intentando resolver en términos más concretos. Dijimos en la sección \ref{sec:problema} que la intención es adaptar Noir para que pueda usar Plonky2 para generar pruebas de ejecución. Concretamente, el trabajo consiste en hacer un programa (el cual llamaremos ''Noirky2'' de ahora en adelante) que tiene como dependencias a Nargo (el compilador de Noir) y a Plonky2. Tanto Nargo como Plonky2 están escritos en Rust, por lo tanto Noirky2 también estará programado en Rust. La figura \ref{fig:esquema_noirky2} muestra un esquema en alto nivel de la interacción de los programas involucrados.
...
% Nodes
\node[box] (noir_) {Noir};
\node[box, right=of noir_] (acir_) {ACIR};
\node[redbox, right=1.5cm of acir_, yshift=1cm] (intermediate_) {Noirky2};
\node[box, right=1.5cm of intermediate_] (plonky2_) {Plonky2};
\node[box, below=1.5cm of plonky2_] (barretenberg_) {Barretenberg};

% Arrows
%\draw[->] (noir_) -- (acir_);
%\draw[->] (acir_) -- (intermediate_);
%\draw[->] (intermediate_) -- (plonky2_);
%\draw[->] (plonky2_) -- (intermediate_);
%\draw[->] (acir_.south) |- (barretenberg_.west);
...
\begin{itemize}
    ...
    \item Responder a los mismos comandos que Barretenberg provee: \textbf{prove}, \textbf{create\_vk} y \textbf{verify}. La semántica de estos debe ser la misma que en el otro prover.
\end{itemize}
...
\section{Tiempos de ejecución}\label{sec:tiempos_de_ejecución}
Hay un concepto que es tan importante para este proyecto que merece una sección aparte. Noirky2 cuenta con 3 momentos de ejecución distintos. Entender la diferencia entre ellos es crucial para entender conceptualmente donde estamos parados y qué recursos tenemos en cada momento:
\begin{enumerate}
...
\begin{itemize}
    ...
    \item Una lista ordenada $OP$ de opcodes (AssertZero, operaciones de memoria, BlackBoxFunction y BrilligCall). Notamos $OP[i]$ al i-esimo opcode. Decimos también que $w_i \in OP[j]$ si el witness $w_i$ es referenciado por el j-esimo opcode del código ACIR.
\end{itemize}
...
que asigna a cada witness del código ACIR $P$ un target del circuito de Plonky2 $P'$. Concretamente, esta función va a ser implementada con un diccionario que tendremos que mantener durante toda la generación del circuito. ¿Por qué es necesario esto? Hay varios motivos para tener este mapeo:
\begin{enumerate}
    \item Supongamos que en el código ACIR tenemos el listado $OP$ de opcodes y que $$\exists w \in W: ~ w \in OP[i] \land w \in OP[j] ~~ i < j$$ 
...
\subsubsection{\textbf{Asignación condicional en Plonky2}}

La parte más complicada está relacionada con la lectura y la modificación de los bloques de memoria, que veremos a continuación. Pero antes, veamos cómo podemos implementar una asignación condicional en este modelo. Más concretamente, nos interesa tener una restricción de la forma
\[
...
\]
donde \texttt{equal} también es un target booleano.
...
Nuevamente podríamos preguntarnos sobre la eficiencia de estas operaciones, y nuevamente vamos a postergar el tema. 
...
$$0 \leq w < 2^{num\_bits}$$
La existencia de este opcode particular se justifica con la dificultad para expresar la restricción con ecuaciones polinomiales. Una solución alternativa podría ser expresar esto mismo con varias operaciones de AssertZero, pero la filosofía de Noir en estos casos es delegar operaciones complejas al prover. 
...
Es importante destacar que los valores de $b_i$ van a ser \textbf{pistas} para el sistema de ecuaciones, calculadas por Plonky2 en tiempo de resolución de circuito en base al valor concreto que se le asigne a $t$. Le vamos a indicar a Plonky2 que lo descomponga en una cantidad fija de elementos, y este va a intentar hacerlo partiendo del bit menos significativo ($b_0$), sin embargo va a fallar si es incapaz de completar la descomposición el número. 
...
También hay que tener en cuenta el overhead de tener la lookup table: las consultas son computacionalmente baratas pero la creación de la tabla puede ser costosa para Plonky2. Especialmente se puede dar una situación donde tengamos un programa pequeño y una lookup table muy grande, en cuyo caso quizás no vale la pena. La idea es entrar más en detalle con estos temas cuando hagamos un análisis y comparación de la performance de los distintos métodos.
...
\begin{itemize}
    ...
    \item se usan de la siguiente forma: en tiempo de generación de circuito se instancian con targets (de entrada y de salida), haciendo que quede pre-seteado el cálculo en tiempo de resolución de circuito.
    \item no son suficientes para garantizar un circuito correcto: los Generators generan pistas para el prover a través de calcular valores que van a llenar la traza, pero no verifican polinomialmente que los resultados calculados sean correctos. Si simplemente confiamos en que el Generator va a calcular el valor pero no restringimos sus operaciones, estaríamos creando una vulnerabilidad en el sistema. 
\end{itemize}
...
\State \textbf{let mut} \texttt{full\_number\_value = witness\_value\_mapping.get(full\_number\_target)}
\For{\texttt{i = 0..N}}
...
Internamente, Plonky2 cuenta con un \texttt{witness\_value\_mapping} donde almacena los valores resueltos de los targets. Ese diccionario es el que usamos para almacenar los nuevos valores y obtener los valores de las dependencias. Un detalle curioso es que si tratamos de sobre-escribir el valor de alguno de los targets, el sistema generará un error en tiempo de resolución de circuito ya que un target no puede tomar 2 valores distintos.

Más allá de la ejecución del Generator, mencionamos anteriormente que el circuito tiene además que generar las restricciones polinomiales para la ejecución realizada, por lo tanto, debemos establecer una restricción de la forma
