...
\begin{enumerate}
    ...
    \end{itemize}
    En este punto el directorio debería ser algo como esto:
    ...
    \item El siguiente paso sería generar la prueba de ejecución de este circuito con algún proving system. 
    Para esto vamos a usar un prover. El único prover que sabe interpretar código ACIR es \textbf{Barretenberg}, por lo cual tendremos que instalarlo. Lo siguiente que tenemos que hacer es generar la prueba de ejecución de Barretenberg, con el comando:
    ...
    Esto va a crear una prueba ($\pi$) de Barretenberg en \texttt{nombre\_proyecto/proof}.
    
    \item A continuación queremos verificar la prueba. Esto consta de 2 partes: generar la clave de verificación y verificarla. En un caso real la verificación querríamos que sea hecha por un agente distinto al que generó la prueba, por lo tanto, tanto la clave de verificación como la prueba deberían ser pasadas al verifier.
...
Un opcode AssertZero determina de forma explícita una restricción polinomial sobre los witnesses. Internamente está compuesta por una \texttt{Expression}. Esta expresión tiene 3 campos posibles:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{value}: un witness que contiene el valor que se quiere escribir en el arreglo. 
\end{itemize}
Podemos pensar una operación de lectura como una restricción de la forma
$$B_{blockId}[w_{index}] := w_{value}$$ 
Veamos nuevamente un ejemplo minimal con un programa de Noir, como el que se puede ver en la figura \ref{fig:noir_ejemplo_mem_write}. Este programa recibe nuevamente un arreglo de tamaño 4 y un índice público, pero también un valor público que es el que será escrito en el arreglo. Este programa escribe el valor $val$ en la posición $idx$ y finalmente retorna el valor escrito. 
