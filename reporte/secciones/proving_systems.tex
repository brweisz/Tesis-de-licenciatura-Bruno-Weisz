...
\begin{itemize}
    ...
    \item \textbf{Non-interactive}: esta propiedad nos habla de la ausencia de interactividad en el protocolo, es decir, una prueba puede ser generada de forma offline y luego ser verificada en cualquier momento del futuro. Dijimos que los protocolos SNARK eran interactivos, pero esta interactividad puede ser removida con la heurística de Fiat-Shamir, que será vista en la sección \ref{sec:shamir}.
...
Hasta ahora vimos algunas configuraciones que pueden tener filas de la matriz $Q$, en particular para operaciones básicas como sumar y multiplicar. Sin embargo, PLONK nos permite definir compuertas personalizadas, customizando como queramos la matriz $Q$ para conseguir la operación deseada. En particular, vemos que podemos comprimir todo nuestro programa de ejemplo en una sola fila de la matriz Q, quedándonos de esta manera un conjunto de matrices como se puede ver en \ref{tab:matrices_completas_comprimidas}.
...
\textbf{Vamos a interpolar cada una de las columnas} sobre el dominio $D$. Esto nos va a dejar como resultado los 9 polinomios interpoladores, los cuales vamos a nombrar respectivamente (al igual que sus funciones asociadas en $\Fp$) $q_L$, $q_R$, $q_M$, $q_O$, $q_C$, $a$, $b$, $c$ y $pi$. Conceptualmente, lo que acabamos de hacer es crear 9 polinomios que cumplen una propiedad muy interesante. Tomemos por ejemplo el polinomio $a$, obtenido a partir de interpolar los valores de la columna $A$. Sin perder generalidad, vamos a notar $A[i]$ al valor en la i-ésima fila de la columna $A$. Dado el dominio de interpolación elegido y los valores que estamos interpolando, podemos ver que por definición se cumple la igualdad
...
Sin embargo, Turbo-PLONK permite la definición de ecuaciones distintas, es decir, salirse del esquema definido por la matriz $Q$ y definir ecuaciones \textbf{arbitrarias}. En este esquema, la traza no se ve limitada a las columnas $A$, $B$ y $C$ sino que el estado del programa tiene un ancho arbitrario $\omega$ definido por el usuario o el protocolo. A su vez, la matriz $Q$ tiene otra semántica: ahora va a tener un ancho arbitrario y cada columna representará una \textit{custom gate}, con un $1$ si está encendida y un $0$ si está apagada. En otras palabras, la matriz Q nos permite encender y apagar restricciones polinomiales en cada fila. 

Dado un estado $v \in \Fp^\omega$, una traza válida de Turbo-PLONK es una matriz $T \in \Fp^{\omega \times n}$ que cumple con la función de transición $P$ y respeta las restricciones impuestas por la matriz $V$. La función de transición $P$ es un polinomio de grado a lo sumo $d$ en $2\omega + l$ variables, donde $l$ es el número de selectores (por ende, el ancho de la matriz $Q$). Estos selectores nos permiten ''prender'' y ''apagar'' compuertas. La restricción establecida sobre $P$ es la siguiente:
