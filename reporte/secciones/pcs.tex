...
Un punto de curva se representa a través de un par ordenado de componentes, tal que $(x,y)\in \Fp \times \Fp$. Recordemos que usando una curva como BLS12-381, $p$ es un número primo de 381 bits. Por ende, un punto de curva se representa con 762 bits. Una aclaración importante es que para almacenar un punto de curva alcanza con guardar el valor de $x$ y usar un bit para determinar si $y$ es positivo o negativo. Esto se debe a que dado un valor de $x$ y la ecuación de la curva, queda determinado el valor de $y$. Con esto en mente, podríamos representar un punto de curva de manera comprimida con 382 bits. Para las cuentas posteriores, vamos a tomar la representación explícita (de 762 bits) ya que es la que se usa para realizar operaciones. 
...
\section{FRI}
Otro protocolo que implementa oráculos de polinomios es FRI\cite{ben2018fast}. Este protocolo tiene la ventaja de que no necesita un CRS para funcionar, sin embargo las pruebas que genera ocupan más espacio de almacenamiento que las de KZG para alcanzar el mismo nivel de seguridad. FRI también es un protocolo interactivo, con muchas más interacciones que KZG y que se apoya en los commitments de listas de los Merkle Trees vistos en la sección \ref{sec:merkle_tree}.
