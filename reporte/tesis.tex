...
Por otro lado, también se realizaron tests funcionales que siguen el flujo completo: parten de un programa de Noir, realizan la traducción, generan la prueba y la verifican. Esto se hizo con algunos programas de ejemplo ya presentes en el repositorio de Noir.
...
Todo el material referente al desarrollo de la tesis se puede encontrar en \href{https://github.com/brweisz/Tesis-de-licenciatura-Bruno-Weisz}{este repositorio}. Este contiene a grandes rasgos la implementación de Noirky2, la experimentación que se describirá a continuación y las fuentes del presente reporte.
...
\begin{itemize}
    ...
    \item Barretenberg (con KZG) tarda siempre un tiempo cercano a los $25ms$ mientras que todas las versiones de Noirky2 (con FRI) tardan alrededor de $5ms$. Esta comparación está más relacionada con la implementación de los proving systems en sí que con la implementación realizada en este trabajo. 
\end{itemize}
...
\end{tabular}
\caption{Comparación entre el tamaño del CRS y el tamaño de las pruebas de Plonky2}
...
\begin{itemize}
	...
	\item Analizar en detalle el código de Barretenberg para explicar por qué son tanto más rápidas las operaciones de memoria. También se podría estudiar cómo Barretenberg implementa las operaciones de Range para comparar con la implementación de Noirky2. 
	...
	\item Podrían usarse técnicas de fuzzing para explorar el espacio de programas de Noir y ver si el ACIR generado cambia en algún caso dependiendo del valor de $\Fp$ utilizado.
...
\begin{enumerate}[start=6]
	...
	\item \textbf{Generación de la Verifying Key:}
	
	\vspace{0.5em}
	\texttt{bb write\_vk -b target/nombre\_proyecto.json -o target/vk}
	
	Este comando va a generar una \textbf{Verification Key} en la ruta indicada. Esta contiene toda la información necesaria para verificar la prueba: el programa codificado, los public inputs, etc. En lenguaje de PLONK, podríamos pensarlo como las matrices $Q$, $V$ y $PI$. 
	
	\item \textbf{Verificación de la prueba:}
	
	\texttt{bb verify -k target/vk -p proof}
	\vspace{0.5em}
