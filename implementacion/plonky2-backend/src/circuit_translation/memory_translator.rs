...
impl<'a> MemoryOperationsTranslator<'a> {
    ...
    fn _translate_memory_read(&mut self, block_id: &BlockId, op: &MemOp) {
        ...
        let result_witness = op.value.to_witness().unwrap();
        let memory_block = &self.memory_blocks[block_id].0;

        let mut result_target = self.builder.zero();
        for i in 0..memory_block.len() {
            let i_constant = self.builder.constant(F::from_canonical_usize(i));
            let is_current_position = self.builder.is_equal(i_constant, target_idx_to_read);
            result_target = self.builder._if(is_current_position, memory_block[i], result_target);
        }

        self.witness_target_map.insert(result_witness, result_target);
    }
